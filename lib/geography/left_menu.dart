  ...
  return Container(
    ...
    child: ListView(
      ...
      children: <Widget>[
        ...
        ListTile(
          title: Center(child: Text('edit type')),
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => AttributesEditWidget(type)),
            );
          },
        ),
        ListTile(
...
Future<void> pushLastGame(BuildContext context) async {
  ...
  propertyRows.forEach((row) => gamePropertiesList.add(new GameConstantInstance(
      row[DatabaseGameHelper.propertyTypeColumn],
      row[DatabaseGameHelper.propertyValueColumn])));
  Navigator.push(
    context,
    MaterialPageRoute(
      builder: (context) => SingleGameStatistics(
          lastGameAnwsers: anl, gameProperties: gamePropertiesList),
    ),
  );
}
